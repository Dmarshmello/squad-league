// ============================================================
//  config.js — Supabase client + shared helpers
// ============================================================
...
// Lazy init so CDN is guaranteed ready before createClient is called
let _dbClient = null;
function getDbClient() {
  if (!_dbClient) _dbClient = supabase.createClient(SUPABASE_URL, SUPABASE_ANON);
  return _dbClient;
}
// Proxy so every file can call db.from(...) as normal
const db = new Proxy({}, { get(_, prop) { return getDbClient()[prop]; } });
...
function getWeekNumber(matchDate, seasonStart) {
  ...
  const seasonMonday = getMondayOf(seasonStart && seasonStart !== '' ? seasonStart : matchDate);
...
function avatarHtml(player, cls = 'player-avatar') {
  ...
  const photo = player && player.photo_url;
  if (photo) {
    const safe = name.replace(/'/g, '');
    return `<div class="${cls}"><img src="${photo}" alt="${name}" onerror="this.parentNode.innerHTML='${initials(safe)}'"></div>`;
  }
